    ...
    for (impl_key, conditional_impl) in &types.impls {
        match impl_key {
            ...
            ImplKey::RustOption(ident) => {
                hidden.extend(expand_rust_option(ident, types, conditional_impl));
            }
    ...
    if let Some(ret) = &efn.ret {
        expr = match ret {
            ...
            Type::RustOption(ty) => {
                ...
                match &ty.inner {
                    Type::RustBox(b) => {
                        let inner_ty = &b.inner;
                        let box_ty = quote!(::cxx::alloc::boxed::Box<#inner_ty>);
                        if types.is_considered_improper_ctype(&b.inner) {
                            quote_spanned!(span=> ::cxx::private::RustOption::<#box_ty>::from_raw_improper(#call).into_option())
                        } else {
                            quote_spanned!(span=> ::cxx::private::RustOption::<#box_ty>::from_raw(#call).into_option())
                        }
                    }
            ...
            Type::RustOption(ty) => {
                ...
                match &ty.inner {
                    Type::RustBox(b) => {
                        let inner_ty = &b.inner;
                        let box_ty = quote!(::cxx::alloc::boxed::Box<#inner_ty>);
                        quote_spanned!(span=> ::cxx::private::RustOption::<#box_ty>::from_raw(#var).into_option())
                    }
                    Type::Ref(r) => match &r.inner {
...
fn expand_rust_option(
    ...
    let (key, link_prefix, local_prefix) = match inner {
        OptionInner::RustBox(key) => {
            ...
            (key, link_prefix, local_prefix)
        }
        OptionInner::Ref(key) => {
            let elem = key.rust();
            if types.try_resolve(elem).is_none() {
                return TokenStream::new();
            }
            let resolve = types.resolve(elem);
    ...
    let (ty, trait_impl_ty, ty_ptr, const_ty_ptr, prevent_unwind_drop_label) = match inner {
        OptionInner::RustBox(_) => (
            quote! { ::cxx::alloc::boxed::Box<#inner_with_generics> },
            quote! { ::cxx::alloc::boxed::Box<#inner_with_generics> },
            quote! { *mut #inner_with_generics },
            quote! { *const #inner_with_generics },
            format!("::alloc::boxed::Box<::{}> as Drop>::drop", ident),
        ),
        OptionInner::Ref(_) => (
            quote! { &#inner_with_generics },
            quote! { &#inner_with_generics },
            quote! { *const #inner_with_generics },
            quote! { *const #inner_with_generics },
        ...
        OptionInner::RefVec(_) => (
            quote! { &::cxx::private::RustVec<#inner_with_generics> },
            quote! { &#inner_with_generics },
            quote! { *const ::cxx::private::RustVec<#inner_with_generics> },
            quote! { *const ::cxx::private::RustVec<#inner_with_generics> },
            format!("&::alloc::vec::Vec<::{}> as Drop>::drop", ident),
        ),
        OptionInner::MutRefVec(_) => (
            quote! { &mut ::cxx::private::RustVec<#inner_with_generics> },
            quote! { &mut #inner_with_generics },
            quote! { *mut ::cxx::private::RustVec<#inner_with_generics> },
            quote! { *const ::cxx::private::RustVec<#inner_with_generics> },
    ...
    let set_value_impl = match inner {
        OptionInner::RustBox(_) => quote! {
            ...
            #[doc(hidden)]
            #[unsafe(export_name = #link_value_const)]
    ...
    quote_spanned! {end_span=>
        ...
        #[doc(hidden)]
        #[unsafe(export_name = #link_has_value)]
