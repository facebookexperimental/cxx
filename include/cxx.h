...
public:
  Option() noexcept;
  Option(Option&&) noexcept;
  Option(Vec<T>&&) noexcept;
  ~Option() noexcept;

  Option<Vec<T>>& operator=(Option&&) noexcept;
  const Vec<T>& operator*() const noexcept;
  const Vec<T>* operator->() const noexcept;
  Vec<T>& operator*() noexcept;
  Vec<T>* operator->() noexcept;

  bool has_value() const noexcept;
  const Vec<T>& value() const noexcept;
  Vec<T>& value() noexcept;
  void reset();
  void set(Vec<T>) noexcept;

  // Internal API only intended for the cxxbridge code generator.
  Option(unsafe_bitcopy_t, const Option &) noexcept;
private:
  std::array<std::uintptr_t, 3> repr;
...
#define OPTION_DIRECT_DECL(INNER, REPR, COUNT)                    \
template<>                                                        \
class Option<INNER> {                                             \
public:                                                           \
  Option() noexcept;                                              \
  Option(Option&&) noexcept;                                      \
  Option(INNER&&) noexcept;                                       \
  ~Option() noexcept;                                             \
                                                                  \
  Option<INNER>& operator=(Option&&) noexcept;                    \
  const INNER& operator*() const noexcept;                        \
  const INNER* operator->() const noexcept;                       \
  INNER& operator*() noexcept;                                    \
  INNER* operator->() noexcept;                                   \
                                                                  \
  bool operator==(const Option &) const noexcept;                 \
  bool operator!=(const Option &) const noexcept;                 \
                                                                  \
  bool has_value() const noexcept;                                \
  const INNER& value() const noexcept;                            \
  INNER& value() noexcept;                                        \
  void reset();                                                   \
  void set(INNER) noexcept;                                       \
                                                                  \
  Option(unsafe_bitcopy_t, const Option &) noexcept;              \
private:                                                          \
  std::array<REPR, COUNT> repr;                                                      \
                                                                  \
  void drop() noexcept;                                           \
};

OPTION_DIRECT_DECL(String, std::uintptr_t, 3)
...
template<typename T>
bool Option<const T&>::operator==(const Option& other) const noexcept {
  if (this->has_value() && other.has_value()) {
    return this->value() == other.value();
  }
  return this->has_value() == other.has_value();
...
template<typename T>
bool Option<Box<T>>::operator==(const Option& other) const noexcept {
  if (this->has_value() && other.has_value()) {
    return this->value() == other.value();
  }
  return this->has_value() == other.has_value();
...
//////// Option<Vec<T>> ////////////
template <typename T>
Option<Vec<T>>::Option(Option&& other) noexcept {
  new (this) Option();
  if (other.has_value()) {
    set(std::move(other.value()));
  }
...
template <typename T>
Option<Vec<T>>::Option(Vec<T>&& value) noexcept {
  new (this) Option();
  set(std::move(value));
}

template <typename T>
Option<Vec<T>>::~Option() noexcept {
  this->drop();
}

template <typename T>
Option<Vec<T>>& Option<Vec<T>>::operator=(Option&& other) noexcept {
  this->reset();
  if (other.has_value()) {
    set(std::move(other.value()));
  }
...
template <typename T>
const Vec<T>& Option<Vec<T>>::operator*() const noexcept {
  return value();
}
...
template <typename T>
Vec<T>& Option<Vec<T>>::operator*() noexcept {
  return value();
}
...
template <typename T>
void Option<Vec<T>>::reset() {
  this->drop();
  new (this) Option();
}

template <typename T>
Option<Vec<T>>::Option(unsafe_bitcopy_t, const Option &bits) noexcept : repr(bits.repr) {}
#endif // CXXBRIDGE1_RUST_OPTION
