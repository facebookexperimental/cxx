...
macro_rules! rust_option_shims {
    ($segment:expr, $ty:ty) => {
        ...
        const _: () = {
            ...
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$", $segment, "$set")]
            ...
            #[export_name = concat!("meta_cxxbridge1$rust_option$", $segment, "$new")]
            unsafe extern "C" fn __new(this: *mut RustOption<&mut $ty>) {
                unsafe { ptr::write(this, RustOption::new()) }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$", $segment, "$drop")]
            unsafe extern "C" fn __drop(this: *mut RustOption<&mut $ty>) {
                unsafe { ptr::drop_in_place(this) }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$", $segment, "$has_value")]
            unsafe extern "C" fn __has_value(this: *mut RustOption<&mut $ty>) -> bool {
                unsafe { this.as_ref().unwrap().value().is_some() }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$", $segment, "$value_ptr")]
            unsafe extern "C" fn __value_ptr(this: *mut RustOption<&mut $ty>) -> *mut &mut $ty {
                unsafe { this.as_mut().unwrap().as_ref_mut_inner_unchecked() as _ }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$", $segment, "$set")]
        ...
        const _: () = {
            /* Vec<T> impl */
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$rust_vec$", $segment, "$new")]
            unsafe extern "C" fn __const_new(this: *mut RustOption<&RustVec<$ty>>) {
                unsafe { ptr::write(this, RustOption::new()) };
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$rust_vec$", $segment, "$drop")]
            unsafe extern "C" fn __const_drop(this: *mut RustOption<&RustVec<$ty>>) {
                unsafe { ptr::drop_in_place(this) }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$rust_vec$", $segment, "$has_value")]
            unsafe extern "C" fn __const_has_value(this: *mut RustOption<&RustVec<$ty>>) -> bool {
                unsafe { this.as_ref().unwrap().value().is_some() }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$rust_vec$", $segment, "$value_ptr")]
            unsafe extern "C" fn __const_value_ptr(this: *mut RustOption<&RustVec<$ty>>) -> *mut &RustVec<$ty> {
                unsafe { this.as_mut().unwrap().as_ref_mut_inner_unchecked() as _ }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$const$rust_vec$", $segment, "$set")]
            unsafe extern "C" fn __const_set<'__cxx>(
            ...
            #[export_name = concat!("meta_cxxbridge1$rust_option$rust_vec$", $segment, "$new")]
            unsafe extern "C" fn __new(this: *mut RustOption<&mut RustVec<$ty>>) {
                unsafe { ptr::write(this, RustOption::new()) }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$rust_vec$", $segment, "$drop")]
            unsafe extern "C" fn __drop(this: *mut RustOption<&mut RustVec<$ty>>) {
                unsafe { ptr::drop_in_place(this) }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$rust_vec$", $segment, "$has_value")]
            unsafe extern "C" fn __has_value(this: *mut RustOption<&mut RustVec<$ty>>) -> bool {
                unsafe { this.as_ref().unwrap().value().is_some() }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$rust_vec$", $segment, "$value_ptr")]
            unsafe extern "C" fn __value_ptr(this: *mut RustOption<&mut RustVec<$ty>>) -> *mut &mut RustVec<$ty> {
                unsafe { this.as_mut().unwrap().as_ref_mut_inner_unchecked() as _ }
            }
            #[export_name = concat!("meta_cxxbridge1$rust_option$rust_vec$", $segment, "$set")]
            unsafe extern "C" fn __set<'__cxx>(
